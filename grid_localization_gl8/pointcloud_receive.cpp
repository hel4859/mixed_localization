    ...
    if(!USE_GLOBALPOINTCLOUDFORMATCHING)
    {
        if(SAVE_POINTCLOUD)
        {
            ...
            if((tt>timeForGps) && (step % SAVE_POINTCLOUDSTEP == 0) && (DR.velocity!=0))
            {
                ...
                if (globalPointsMap.size()> 6000000)
                {
                    stringstream ss;
                    ss<<i;

                    string s1="/home/cyber/gl8_ws/src/localization/grid_localization_gl8/pointcloud/new";
                    string s2 = ss.str();
                    string s3=".ply";
                    std::string filename=s1+s2+s3;
                    i++;
                    ROS_INFO("ply file saved successfully");
                    globalPointsMap.saveToPlyFile(filename,1);
