...
pointcloud_receive::pointcloud_receive()
{
    ...
    pnh.param<string>("pointcloud_file", pointcloud_file,
                      "/home/hl/catkin_ws/src/grid_localization/pointcloud/cyberfly_g200_oct_cloud.ply");
    pnh.param<string>("pointcloud_save", pointcloud_save,
                      "/home/hl/catkin_ws/src/grid_localization_gl8/pointcloud/");
    pnh.param<string>("gridmap_path", gridmap_path,
                      "/home/hl/catkin_ws/src/grid_localization/map/cyberfly_g200_oct");
    pnh.param<string>("log_file_path", log_file_path,
                      "/home/hl/catkin_ws/src/grid_localization/save_log");
    pnh.param<string>("config_file_path", config_file_path,
                      "/home/hl/catkin_ws/src/grid_localization/map/config");

    pnh.getParam("excute_mode",excute_mode);
    pnh.getParam("pointcloud_file",pointcloud_file);
    pnh.getParam("pointcloud_save",pointcloud_save);
...
//    sync->registerCallback(boost::bind(&pointcloud_receive::pointcloud_callback, this, _1, _2));
    ...
    gps_pose = nh.subscribe("gps_pose",20,&pointcloud_receive::gps_callback,this);
...
void pointcloud_receive::gps_callback(const nav_msgs::Odometry &msg) {

    gps_x = msg.pose.pose.position.x;
    gps_y = msg.pose.pose.position.y;
    gps_z = msg.pose.pose.position.z;
    tf::Matrix3x3(tf::Quaternion(msg.pose.pose.orientation.x, msg.pose.pose.orientation.y,
                                 msg.pose.pose.orientation.z, msg.pose.pose.orientation.w)).getRPY(gps_rx, gps_ry, gps_rz);

  //  std::cout<<"oriention:"<<gps_rx<<" "<<gps_ry<<" "<<gps_rz<<std::endl;
}
...
//        curPointsMapColoredTemp.insertAnotherMap(&curPointsMapColoredRear,CPose3D(0,0,0,0,0,0));
        ...
        curPointsMapColoredTemp.extractPoints(
            TPoint3D(min_x, min_y, pointsMap_heightMin),//对高度进行了处理
        ...
        if(SAVE_POINTCLOUD)
        {
            ...
            if((tt>timeForGps) && (step % SAVE_POINTCLOUDSTEP == 0) && (DR.velocity!=0))
            {
                globalPointsMap.insertAnotherMap(
                        &curPointsMapColored,
                        CPose3D(gps_x,gps_y,gps_z,gps_rz,0,0)//100hz 叠加
                        // CPose3D(poseEkf2D.x(),poseEkf2D.y(),0,poseEkf2D.phi(),0,0) //用gps结果叠加点云(把结果改成一姐部分)
                ...
                if (globalPointsMap.size()> 5000000)
                {
                    ...
                    string s2 = ss.str();
                    string s3=".ply";
                    std::string filename = pointcloud_save + s1 + s2 + s3;
                    i++;
